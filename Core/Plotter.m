classdef Plotter
    ...
    methods(Static)
        ...
        % @brief 打印磁力计拟合矫正结果
        % @param stateCalculator实例
        function plot_MagCali(stateCalculator)
            gcf = figure('Name', '磁力计矫正结果');
            mseq = stateCalculator.iHandler.sensorCali.magCalSeq;
            magCenter = stateCalculator.iHandler.sensorCali.magCalCenter';
            plot3(mseq(:,1), mseq(:,2), mseq(:,3), 'b.');hold on;
            plot3(magCenter(1), magCenter(2), magCenter(3), 'ro', 'MarkerSize', 10, 'LineWidth', 2);
            axis equal
            xlabel('X');
            ylabel('Y');
            zlabel('Z');
...
function gcf = plot_Tracks_Square(stateCalrArray,varargin)
    ...
    addParameter(settings,'xmin',-25); %
    addParameter(settings,'xmax',5); %
    addParameter(settings,'ymin',-5); %
    addParameter(settings,'ymax',22); %
    addParameter(settings,'exprName',""); % 实验名称
    ...
    xmin = settings.Results.xmin;
    xmax = settings.Results.xmax;
    ymin = settings.Results.ymin;
    ymax = settings.Results.ymax;
    ...
    plot(P1_adjust(1,:),P1_adjust(2,:),'r-','LineWidth',2,'DisplayName','Prop. ZUPT'); hold on;

    % 绘制Traditional
    P2 = stateCalrArray(2).mStateSeq.P';
    P2_adjust = TrackAdjuster.rotate2D(P2(1,:),P2(2,:),P2_angle);
    plot(P2_adjust(1,:),P2_adjust(2,:),'b-','LineWidth',2,'DisplayName','Trad. ZUPT'); hold on;

    % 绘制参考轨迹
    plot([0,0,-16,-16,0],[0,19.5,19.5,0,0],'g--','LineWidth',2,'DisplayName','Ref. Path');

    % 绘制结束点
    plot(P1_adjust(1,end),P1_adjust(2,end),'ro','LineWidth',2,'DisplayName','Prop. End'); hold on;
    plot(P2_adjust(1,end),P2_adjust(2,end),'bo','LineWidth',2,'DisplayName','Trad. End'); hold on;
    ...
    % 添加图例
    lgd = legend('show','Location', 'northeast'); % 图例位置
    ...
    set(gca, 'FontSize', 12); % 设置刻度值字体大小为12
        ...
        function gcf = plot_Tracks_Line(stateCalrArray,varargin)
            ...
            for i = 1:length(stateCalrArray)
                % 绘制
                P1 = stateCalrArray(i).mStateSeq.P';
                P1_adjust = TrackAdjuster.rotateByMean(P1);
                str = "track " + num2str(i);
...
%             legend('show');
        ...
        function gcf = plot_Tracks_Playground(stateCalrArray,varargin)
            ...
            % 左半圆
            theta = linspace(pi/2, 3*pi/2, 50); % 从π/2到3π/2的半圆
            x_left = r * cos(theta);
            y_left = r * sin(theta) + L/2;

            % 右半圆
            x_right = r * cos(theta);
            y_right = r * sin(theta) - L/2;

            % 连接左右半圆的直线段
            x_straight_top = [-r, r];
            y_straight_top = [L/2, L/2];
            x_straight_bottom = [-r, r];
            y_straight_bottom = [-L/2, -L/2];

            % 绘制跑道
            plot(x_left, y_left, 'g--', 'LineWidth', 2, 'DisplayName', 'Reference Track');
            plot(x_right, y_right, 'g--', 'LineWidth', 2, 'HandleVisibility', 'off'); % 隐藏重复图例
            plot(x_straight_top, y_straight_top, 'g--', 'LineWidth', 2, 'HandleVisibility', 'off');
            plot(x_straight_bottom, y_straight_bottom, 'g--', 'LineWidth', 2, 'HandleVisibility', 'off');
        ...
        % @param trackArray数组
        % @retval None
        function gcf = plot_Tracks_Playground_directly(tarckArray,varargin)
            %解析参数
            settings = inputParser;
            addParameter(settings,'P1_angle',50); % P1的旋转角度
            addParameter(settings,'P2_angle',50); % P2的旋转角度
            ...
            tarckArray = Plotter.toArray(tarckArray);
